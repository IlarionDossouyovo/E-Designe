    ...
    <div style={{ padding: '2rem 20px', maxWidth: '1400px', margin: '0 auto', background: '#0a0a0f', minHeight: '100vh' }}>
      ...
      <div style={{ 
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>👶 Collection Bebe</h1>
          <p style={{ fontSize: '1.2rem', opacity: 0.9, maxWidth: '500px' }}>
            Douceur et confort pour votre bebe. Vetements doux et tendances.
          </p>
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
            <Link to="/products?cat=Bebe&qual=Premium" style={{ padding: '12px 24px', background: '#16161f', color: '#ff9f43', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>Premium</Link>
          ...
          <Link key={product.id} to={`/products/${product.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ background: '#16161f', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', transition: 'transform 0.3s, box-shadow 0.3s', height: '100%' }} className="product-card">
              <div style={{ height: '280px', overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ height: '100%', background: 'linear-gradient(135deg, #ff9f43 0%, #ee5a24 100%)', display: 'none', alignItems: 'center', justifyContent: 'center', fontSize: '4rem' }}>👶</div>
              ...
              <div style={{ padding: '16px' }}>
                <p style={{ fontSize: '12px', color: '#ff9f43', marginBottom: '4px' }}>{product.sub}</p>
                <h3 style={{ margin: '0 0 8px', fontSize: '1.1rem', fontWeight: '600' }}>{product.name}</h3>
                <p style={{ fontSize: '1.4rem', fontWeight: 'bold', color: '#ff9f43' }}>{product.price} €</p>
              </div>
            </div>
          </Link>
        ))}
