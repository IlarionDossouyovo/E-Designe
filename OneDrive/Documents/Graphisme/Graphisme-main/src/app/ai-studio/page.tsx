                ...
                <div key={index} className="relative group">
                  {file.type === 'image' ? (
                    <div className="w-16 h-16 rounded-lg overflow-hidden border border-white/10">
                      <Image src={file.url} alt={file.name} width={64} height={64} className="w-full h-full object-cover" />
                    </div>
                  ) : (
        ...
        {mockups.map((mockup) => (
          ...
          >
            <div className={`h-24 rounded-lg bg-gradient-to-br ${mockup.bg} mb-3 flex items-center justify-center`}>
              {image ? (
                <Image src={image} alt={mockup.name} width={200} height={96} className="w-full h-full object-cover rounded-lg opacity-60" />
              ) : (
