...
const hommeProducts = Object.values(products.homme).flat()

export default function Homme() {
  const [filterQual, setFilterQual] = useState('Tous')
  const [filterSub, setFilterSub] = useState('Tous')
  
  const subCats = ['Tous', ...new Set(hommeProducts.map(p => p.sub))]
  
  const filtered = hommeProducts.filter(p => {
  ...
  return (
    <div style={{ padding: '2rem 20px', maxWidth: '1200px', margin: '0 auto' }}>
      <h1 style={{ marginBottom: '0.5rem' }}>👔 Homme</h1>
      <p style={{ marginBottom: '2rem', color: '#666' }}>Collection masculine Elegante et Raffinee</p>
          ...
          <Link key={product.id} to={`/products/${product.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              <div style={{ height: '200px', overflow: 'hidden' }}>
                ...
                <div style={{ height: '100%', background: '#1e3a5f', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem' }}>👔</div>
              </div>
